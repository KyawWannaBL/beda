...
import { MOCK_TOWNSHIPS, ROUTE_PATHS } from '@/lib/index';
...
export default function PickupFlow() {
  ...
  const codAmountRef = useRef<HTMLInputElement>(null);

  // Focus-forward: when reaching details step, focus Pieces; when COD toggled, focus COD amount.
  useEffect(() => {
    if (currentStep === 3) setTimeout(() => piecesRef.current?.focus(), 50);
  }, [currentStep]);

  useEffect(() => {
    if (currentStep === 3 && details.codEnabled) setTimeout(() => codAmountRef.current?.focus(), 50);
  }, [currentStep, details.codEnabled]);
  ...
  const handleTagScan = (code: string) => {
    ...
    if (validation.valid) {
      setTtId(code);
      notify('success', `Tag ${code} valid and assigned.`);
      setCurrentStep(2);
    } else {
      notify('error', validation.error || 'Invalid Tag');
    }
  };
  ...
  const handlePhotoCapture = (photo: string) => {
    if (photos.length < 3) {
      ...
      notify('success', `Photo ${photos.length + 1}/3 captured`);
    }
  };
  ...
  const handleFinalConfirm = async () => {
    ...
    try {
      ...
      useTag(ttId);
      notify('success', 'Pickup confirmed successfully!');
      navigate(ROUTE_PATHS.DASHBOARD);
    } catch {
      notify('error', 'Failed to confirm pickup. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex items-center justify-between mb-8 px-2">
      {STEPS.map((step, idx) => (
        <React.Fragment key={step.id}>
          ...
          {idx < STEPS.length - 1 && (
            <div
              className={`flex-1 h-[2px] mb-6 transition-colors ${
    ...
    <div className="max-w-md mx-auto py-6 px-4 pb-24">
        ...
        <Card className="card-modern">
          ...
          <CardContent className="space-y-4">
            <div className="qr-scanner-frame aspect-square">
              <QRScanner onScan={handleTagScan} expectedType="TT" />
            </div>
        ...
        <Card className="card-modern">
          ...
          <CardContent className="space-y-4">
            <PhotoCapture
              ...
              watermarkData={{
                ttId,
                userId: legacyUser?.id || user?.id || 'unknown',
                timestamp: new Date().toISOString(),
                gps: 'GPS from DeliveryFlow/Map layer',
              }}
            ...
            <div className="flex gap-3 mt-4">
              <Button variant="outline" className="flex-1 h-14" onClick={() => setCurrentStep(1)}>
      ...
      {currentStep === 3 && (
        <Card className="card-modern">
          <CardHeader>
            ...
            <CardDescription>Enter minimal required details for registration queue.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label>Type</Label>
                <Select value={details.type} onValueChange={(v: any) => setDetails({ ...details, type: v })}>
            ...
            <div className="flex items-center justify-between p-4 bg-muted rounded-xl">
              ...
              <Switch checked={details.codEnabled} onCheckedChange={(checked) => setDetails({ ...details, codEnabled: checked })} />
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    type="number"
                    placeholder="0.00"
                    className={`pl-12 ${BRITIUM_BLUE_GUARD}`}
            ...
            <div className="space-y-2">
              ...
              <Select value={details.township} onValueChange={(v) => setDetails({ ...details, township: v })}>
                <SelectTrigger className={BRITIUM_BLUE_GUARD}>
            ...
            <div className="flex gap-3 pt-2">
              <Button variant="outline" className="flex-1 h-14" onClick={() => setCurrentStep(2)}>
                Back
              </Button>
              <Button
                className={`flex-1 ${BRITIUM_BIG_PRIMARY}`}
      ...
      {currentStep === 4 && (
        <div className="space-y-4">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-y-3 text-sm">
                ...
                <span className="font-medium text-right">
                  {details.pieces}x {details.type}
                </span>

                <span className="text-muted-foreground">Condition</span>
                <span
                  className={`font-medium text-right ${
                    details.condition === 'Damaged' ? 'text-destructive' : 'text-emerald-600'
                  }`}
                >
                  {details.condition}
                </span>
              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                {photos.map((p, i) => (
                  <img key={i} src={p} className="h-20 w-20 rounded-md object-cover border" alt="evidence" />
                ))}
              </div>
          ...
          <div className="flex gap-3">
            <Button variant="outline" className="flex-1 h-14" onClick={() => setCurrentStep(3)}>
              Edit Details
            </Button>
            <Button className={`flex-1 ${BRITIUM_BIG_PRIMARY}`} onClick={handleFinalConfirm} disabled={loading}>
      ...
      <footer className="fixed bottom-0 left-0 right-0 p-4 bg-background/80 backdrop-blur-md border-t border-border flex justify-around items-center z-40">
        ...
        <Button
          variant="ghost"
          className="flex flex-col gap-1 h-auto py-2 text-xs"
          onClick={() => navigate((ROUTE_PATHS as any)?.RIDER?.TAGS ?? ROUTE_PATHS.DASHBOARD)}
        >
